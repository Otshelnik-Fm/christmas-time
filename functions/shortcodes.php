...
function chr_counter_load( $arg ) {
	// праздник прошел
	if ( false === chr_is_holiday() ) {
		return '';
	}

	wp_enqueue_style( 'chr-count', rcl_addon_url( 'assets/css/count.css', __FILE__ ), '', CHR_VERSION );
	wp_enqueue_script( 'chr-count', rcl_addon_url( 'assets/js/count.js', __FILE__ ), '', CHR_VERSION );
...
function chr_counter_box( $width = false ) {
	return '<div class="chr_wrapper" style="opacity:0;">
...
// idea https://codepen.io/AngelaVelasquez/pen/QaKOym
add_shortcode( 'chr_tree', 'chr_tree_load' );
function chr_tree_load( $arg ) {
	// праздник прошел
	if ( false === chr_is_holiday() ) {
		return '';
	}

	wp_enqueue_style( 'chr-tree', rcl_addon_url( 'assets/css/tree.css', __FILE__ ), '', CHR_VERSION );
	wp_enqueue_script( 'chr-tree-lib', rcl_addon_url( 'assets/js/tree-lib.js', __FILE__ ), 'chr-tree-lib', CHR_VERSION );
	wp_enqueue_script( 'chr-tree', rcl_addon_url( 'assets/js/tree.js', __FILE__ ), '', CHR_VERSION );

	$color = ( isset( $arg['color'] ) ) ? $arg['color'] : '#fff';

	return chr_tree_box( $color );
}

function chr_tree_box( $color ) {
	return '<div class="chr_tree_container" style="background-color:' . $color . '">
...
// idea https://codepen.io/stephanieogaygarcia/pen/KGbmyK
add_shortcode( 'chr_town', 'chr_town_load' );
function chr_town_load( $arg ) {
	// праздник прошел
	if ( false === chr_is_holiday() ) {
		return '';
	}

	wp_enqueue_style( 'chr-town', rcl_addon_url( 'assets/css/town.css', __FILE__ ), '', CHR_VERSION );
	wp_enqueue_script( 'chr-town', rcl_addon_url( 'assets/js/town.js', __FILE__ ), '', CHR_VERSION );

	$title     = ( isset( $arg['title'] ) ) ? $arg['title'] : 'С новым годом!';
	$title_out = apply_filters( 'chr_town_title', $title );

	$text     = ( isset( $arg['text'] ) ) ? $arg['text'] : 'Наилучшие пожелания веселого Рождества и счастливого Нового года! 🔔🎉🎅🎁🎄';
	$text_out = apply_filters( 'chr_town_text', $text );

	return chr_ball_box( $title_out, $text_out );
}

function chr_ball_box( $title, $text ) {
	/** @noinspection CssUnknownTarget */
	return '<div class="chr-town-container"><div class="animation-container">
  <div class="animation-layer layer-1" style="background-image: url(' . CHR_URL_IMG . 'christmas-back-layer.webp);"></div>
  <div class="animation-layer layer-2" style="background-image: url(' . CHR_URL_IMG . 'christmas-middle-layer-2.webp);"></div>
  <div class="animation-layer layer-3" style="background-image: url(' . CHR_URL_IMG . 'christmas-front-layer-2.webp);"></div>
</div>
<div class="chr-content">
  <div class="chr-content__title">' . $title . '</div>
  <div class="chr-content__text">' . $text . '</div>
</div></div>';
}


// idea https://ukradiolive.com/embed-radio
add_shortcode( 'chr_radio', 'chr_radio_load' );
function chr_radio_load() {
	// праздник прошел
	if ( false === chr_is_holiday() ) {
		return '';
	}

	return '<style>.rcli[class^="fa-"]::before, .rcli[class*=" fa-"]::before {font-family: rcl-awesome !important;}</style>
<div data-my-radio-lang="en" class="embed-radioplayer js-radioplayerData" data-autoplay="true" data-stream="945" data-name="Heart Xmas" data-logo="jpg" data-url="heart-xmas" data-rid="223"><a href="https://ukradiolive.com/heart-xmas" target="_blank">Heart Xmas</a></div><script data-script="jquery.embed-radioplayer.js" async src="https://ukradiolive.com/public/jsv211115112540/jquery.embed-script-loader.js"></script>';
}
